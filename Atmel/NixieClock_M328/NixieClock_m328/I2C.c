	...
	switch(TWI.State){
		...
		case State_I2C_Idle:{
			if (TWI.Enable && TWI.Init){
				TWI.Curr_Ident++;
				if (TWI.Curr_Ident>Max_Ident){
					TWI.Curr_Ident=Min_Ident;
				}
				if((TWI.Ident[TWI.Curr_Ident].Send_Amount || TWI.Ident[TWI.Curr_Ident].Receive_Amount) && TWI.Ident[TWI.Curr_Ident].Ident_given==true){
