...
void UART_Read(uint8_t *_Data_ptr, uint8_t _Amount){
	if (_Data_ptr > RAMSTART && _Data_ptr < RAMEND){//Make sure the pointer points somewhere valid
		UART.Receive.Data_ptr=_Data_ptr;
	}
	if(_Amount < BUFFER_SIZE){//protect from reading beyond the buffer limit
