...
void DS1307_Loop(void){
	switch(DS1307.Status){
		...
		case Status_DS1307_GetDateTime:{
			if (I2C_Ready(DS1307.I2C_Ident) && DS1307.Done==0){//check if the I2C isn't busy
				...
				if (DS1307.Init==INIT_NOK){
					...
					DS1307.Setting.sCH	=(DS1307.buffer[1]&(1<<CH))>0;
					DS1307.Setting.sTWLF=(DS1307.buffer[3]&(1<<TWLF))>0;
					DS1307.Init=INIT_OK;
				}
		...
		case Status_DS1307_GetSettings:{
			if (I2C_Ready(DS1307.I2C_Ident) && DS1307.Done==0){
				...
				DS1307.Setting.sOUT=(DS1307.buffer[1]&(1<<OUT))>0;
				DS1307.Setting.sSQWE=(DS1307.buffer[1]&(1<<SQWE))>0;
				DS1307.Setting.sRS1=(DS1307.buffer[1]&(1<<RS1))>0;
				DS1307.Setting.sRS0=(DS1307.buffer[1]&(1<<RS0))>0;
				if (DS1307.Init==INIT_NOK){
	...
	DS1307.Timer -= DS1307.Timer && Timer_Pulse_1s();
}
...
void DS1307_Set_Time(uint8_t *_Time_Ptr){
	if (DS1307.Command==Status_DS1307_Idle){
		DS1307.DataPtr=_Time_Ptr;
		memcpy(&DS1307.Time.Second, DS1307.DataPtr, sizeof(DS1307.Time));
...
void DS1307_Set_Date(uint8_t *_Date_Ptr){
	if (DS1307.Command==Status_DS1307_Idle){
		DS1307.DataPtr=_Date_Ptr;
		memcpy(&DS1307.Date.Day, DS1307.DataPtr, sizeof(DS1307.Date));
...
void DS1307_Set_DateTime(uint8_t *_DateTime_Ptr){
	if (DS1307.Command==Status_DS1307_Idle){
		DS1307.DataPtr=_DateTime_Ptr;
		memcpy(&DS1307.Time.Second, DS1307.DataPtr, sizeof(DS1307.Time)+sizeof(DS1307.Date));
